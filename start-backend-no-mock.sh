...
# Start the backend service for local development without mock mode
...
if [ $PG_STATUS -ne 0 ]; then
  ...
  echo "For this test, we'll continue anyway but without mock services."
...
echo "Starting backend service on port $PORT without mock mode..."
npm run dev
