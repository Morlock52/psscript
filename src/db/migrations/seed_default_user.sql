...
--
-- For production deployment, run the backend seed-default-user.js script in
-- the hosted backend environment so bcrypt hashes are generated with proper rounds.

DO $$
BEGIN
    IF to_regclass('public.users') IS NULL THEN
        RAISE NOTICE 'Skipping local user seed because public.users does not exist';
        RETURN;
    END IF;
    ...
    INSERT INTO public.users (username, email, password_hash, role, login_attempts, created_at, updated_at)
    SELECT 'admin', 'admin@psscript.local', '$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/X4.Iq0gHvoLf4JyWq', 'admin', 0, NOW(), NOW()
    WHERE NOT EXISTS (
        SELECT 1 FROM public.users WHERE username = 'admin' OR email = 'admin@psscript.local'
    );
    ...
    WHERE NOT EXISTS (
        SELECT 1 FROM public.users WHERE username = 'demo' OR email = 'demo@psscript.local'
    );
END $$;
