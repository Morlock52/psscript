...
CREATE TABLE script_embeddings (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(script_id, model_version)
);
...
-- Documentation storage (crawled docs, tutorials, references)
-- Note: Backend also calls Documentation.sync() on startup, but defining it here
-- keeps local DB setup (src/backend/setup-local-db.js) self-contained.
...
CREATE INDEX idx_documentation_source ON documentation(source);
CREATE INDEX idx_documentation_content_type ON documentation(content_type);
CREATE INDEX idx_documentation_crawled_at ON documentation(crawled_at);
CREATE INDEX idx_documentation_tags_gin ON documentation USING GIN (tags);

-- Command insights (AI-enriched cmdlet reference cards)
-- These are populated by a background enrichment job that scans extracted_commands
-- across the documentation table and produces per-cmdlet details (flags, examples, etc.).
CREATE TABLE command_insights (
    id SERIAL PRIMARY KEY,
    cmdlet_name VARCHAR(200) UNIQUE NOT NULL,
    description TEXT,
    how_to_use TEXT,
...
CREATE INDEX idx_command_insights_cmdlet_name ON command_insights(cmdlet_name);

-- Long-running enrichment jobs for command_insights (progress + cancel).
-- Job IDs are generated by the app (UUID string), so this stays portable without extra extensions.
CREATE TABLE command_enrichment_jobs (
    id VARCHAR(36) PRIMARY KEY,
    status VARCHAR(20) NOT NULL DEFAULT 'queued', -- queued|running|completed|error|cancelled
    request JSONB DEFAULT '{}'::jsonb,
    total INTEGER NOT NULL DEFAULT 0,
    processed INTEGER NOT NULL DEFAULT 0,
    succeeded INTEGER NOT NULL DEFAULT 0,
    failed INTEGER NOT NULL DEFAULT 0,
    current_cmdlet VARCHAR(200),
...
CREATE INDEX idx_command_enrichment_jobs_status ON command_enrichment_jobs(status);
CREATE INDEX idx_command_enrichment_jobs_created_at ON command_enrichment_jobs(created_at);
...
CREATE TRIGGER update_command_insights_updated_at BEFORE UPDATE ON command_insights FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_command_enrichment_jobs_updated_at BEFORE UPDATE ON command_enrichment_jobs FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
