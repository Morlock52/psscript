    ...
    >
      <div className="space-y-6">
        ...
        <div className="rounded-lg border border-[var(--color-border-default)] bg-[var(--color-bg-tertiary)] p-4 space-y-3">
          <h2 className="text-lg font-semibold text-[var(--color-text-primary)]">Install + Support</h2>
        ...
        <div className="rounded-lg border border-[var(--color-border-default)] bg-[var(--color-bg-tertiary)] p-4 space-y-4">
          <h2 className="text-lg font-semibold text-[var(--color-text-primary)]">Backups</h2>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
            <div className="md:col-span-2">
              <label className="block text-xs mb-1 text-[var(--color-text-secondary)]">New backup filename (optional)</label>
              <input
          ...
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-medium text-[var(--color-text-primary)]">Existing backups</h3>
            <button className={buttonSecondary} onClick={() => void loadBackups()} disabled={loading}>
              Refresh
            </button>
          </div>

          <div className="max-h-56 overflow-auto rounded border border-[var(--color-border-default)]">
            <table className="w-full text-sm">
              ...
              <tbody>
                {backups.length === 0 && (
                  <tr>
                    <td className="p-2 text-[var(--color-text-secondary)]" colSpan={3}>
                      No backups found.
                    </td>
                  </tr>
                )}
                {backups.map((b) => (
                  <tr
                    key={b.name}
                    className={`border-t border-[var(--color-border-default)] cursor-pointer ${selectedBackup === b.name ? 'bg-[var(--color-bg-primary)]' : ''}`}
        ...
        <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 p-4 space-y-3">
          <h2 className="text-lg font-semibold text-[var(--color-text-primary)]">Restore Backup</h2>
          <p className="text-sm text-[var(--color-text-secondary)]">
            Restoring replaces current database content with the selected backup.
          </p>

          <div>
            <label className="block text-xs mb-1 text-[var(--color-text-secondary)]">Selected backup</label>
            <select
              className={inputStyles}
          ...
          <div>
            <label className="block text-xs mb-1 text-[var(--color-text-secondary)]">Type exactly: RESTORE BACKUP</label>
            <input
              className={inputStyles}
        ...
        <div className="rounded-lg border border-red-500/40 bg-red-500/10 p-4 space-y-3">
          <h2 className="text-lg font-semibold text-[var(--color-text-primary)]">Clear Test Data</h2>
          <p className="text-sm text-[var(--color-text-secondary)]">
          ...
          {backupBeforeClear && (
            <div>
              <label className="block text-xs mb-1 text-[var(--color-text-secondary)]">Backup filename (optional)</label>
              <input
                className={inputStyles}
          ...
          <div>
            <label className="block text-xs mb-1 text-[var(--color-text-secondary)]">Type exactly: CLEAR TEST DATA</label>
            <input
              className={inputStyles}
              value={clearConfirm}
          ...
          <div>
            <label className="block text-xs mb-1 text-[var(--color-text-secondary)]">Optional table filter (comma-separated)</label>
            <input
              className={inputStyles}
              value={clearTables}
