...
if [ $? -eq 0 ]; then
    ...
    CONVERSATION_HISTORY_COUNT=$(psql "$DATABASE_URL" -t -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'conversation_history';")
    ...
    TOOL_EXECUTION_RESULTS_COUNT=$(psql "$DATABASE_URL" -t -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'tool_execution_results';")
    ...
    AGENT_PERFORMANCE_VIEW_COUNT=$(psql "$DATABASE_URL" -t -c "SELECT COUNT(*) FROM information_schema.views WHERE table_name = 'agent_performance_view';")
    ...
    TOOL_PERFORMANCE_VIEW_COUNT=$(psql "$DATABASE_URL" -t -c "SELECT COUNT(*) FROM information_schema.views WHERE table_name = 'tool_performance_view';")
    ...
    CLEANUP_OLD_AGENT_STATES_COUNT=$(psql "$DATABASE_URL" -t -c "SELECT COUNT(*) FROM pg_proc WHERE proname = 'cleanup_old_agent_states';")
    ...
    CLEANUP_OLD_TOOL_RESULTS_COUNT=$(psql "$DATABASE_URL" -t -c "SELECT COUNT(*) FROM pg_proc WHERE proname = 'cleanup_old_tool_results';")
    ...
    GET_RECENT_CONVERSATIONS_COUNT=$(psql "$DATABASE_URL" -t -c "SELECT COUNT(*) FROM pg_proc WHERE proname = 'get_recent_conversations';")
