...
show_message "2. Backend: cd src/backend && DATABASE_URL=\"\$DATABASE_URL\" DB_PROFILE=supabase DB_SSL=true npm run dev"
show_message "3. AI Service: cd src/ai && source venv/bin/activate && DATABASE_URL=\"\$DATABASE_URL\" DB_SSL=true python -m uvicorn main:app --reload"
show_message "\nUse npm run netlify:build to validate the Netlify frontend build."
